...
def chat_completion_request(key, messages, model="gpt-4o-mini", functions=None, function_call=None,stop=None, **args):
...
class GPTFactory:
    ...
    def predict(self, **gpt_args):
        ...
        for _ in range(self.TRY_TIME):
            if _ != 0:
                time.sleep(15)
            json_data = chat_completion_request(self.openai_key, self.conversation_history, self.model, **gpt_args)
            try:
